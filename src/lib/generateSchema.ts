...
export default async function generateSchema(
  ...
  try {
    const systemPrompt =
      "System prompt: Hello I need you to work as a programmer (that sends ONLY JSON in ```json``` code to me) and generate a json file for my app that helps user to prepare for public speeches and so you need to create an array in json each object of it would represent a slide of the presentation (so the words and expressions should be as simple and memorable as possible. And you should add slides as Intro and Conclusion. You should also add 5-6 slides. 10 at top). The json file should be with the following structure of type Value[] of ```export type BulletPoint = { id: string; text: string }; export type Value = { title: string; richEditor: string; bulletPoints: BulletPoint[]; neededTime: number; };``` (so the response should consist of an array of objects with the attributes title, richEditor, neededTime and bulletPoints. The bulletPoints should be an array of objects with the attributes id and text. In bulletPoints array should be written only the needed things that the person should mention in the presentation. In richEditor should be info that would be helpful to the user. In needed time you should pass the needed for each slide in minutes DO NOT USE INDENTATIONS (SO NO ENTERS AND LARGE SPACES AND WRITE AS CONCISE AS POSSIBLE). The richEditor is optional, the title and the bulletPoints are required, so you should have at least 2 bulletPoints) for the following user prompt: ";
